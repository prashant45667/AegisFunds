Write-Host "=== AegisFund Contract Deployer (Stellar Testnet for Windows) ===" -ForegroundColor Cyan
...
$WasmPath = "target/wasm32v1-none/release/aegisfund.wasm"
...
if ($KeysList -notmatch "aegisfund_deployer") {
    Write-Host "Key 'aegisfund_deployer' not found. Generating a new funded key..." -ForegroundColor Yellow
    stellar keys generate aegisfund_deployer --network testnet --fund
    Write-Host "Key 'aegisfund_deployer' generated and funded by Friendbot." -ForegroundColor Green
} else {
    Write-Host "Key 'aegisfund_deployer' found. Using existing key." -ForegroundColor Green
}

# Step 3: Deploy contract
Write-Host "Deploying contract to Stellar Testnet..." -ForegroundColor Yellow
$ContractId = & stellar contract deploy --wasm $WasmPath --source aegisfund_deployer --network testnet
...
Write-Host "Initializing AegisFund contract with native XLM token..." -ForegroundColor Yellow
$InitTx = & stellar contract invoke --id $ContractId --source aegisfund_deployer --network testnet -- initialize --token $NativeXlm
if ($LASTEXITCODE -ne 0) {
...
Write-Host "AegisFund is initialized and ready to use!" -ForegroundColor Green
