...
echo "=== AegisFund Contract Deployer (Stellar Testnet) ==="

# Step 1: Build the contract to WASM
echo "Building Rust smart contract..."
cargo build --target wasm32-unknown-unknown --release

WASM_PATH="target/wasm32-unknown-unknown/release/aegisfund.wasm"
if [ ! -f "$WASM_PATH" ]; then
...
if [[ ! "$KEYS_LIST" =~ "aegisfund_deployer" ]]; then
    echo "Key 'aegisfund_deployer' not found. Generating a new funded key..."
    stellar keys generate aegisfund_deployer --network testnet
    echo "Key 'aegisfund_deployer' generated and funded by Friendbot."
else
    echo "Key 'aegisfund_deployer' found. Using existing key."
fi
...
echo "Initializing AegisFund contract with native XLM token..."
...
echo "AegisFund is initialized and ready to use!"
