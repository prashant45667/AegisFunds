...
function App() {
  ...
  useEffect(() => {
    ...
    const simMode = localStorage.getItem('aegisfund_sim_mode') === 'true';
    if (simMode) {
      let mockAddr = localStorage.getItem('aegisfund_mock_address') || 'GCHHHKNWLK6KGAVIQD5UEZ3NDLGF4POQVABLL2M3WUR5GVGKOQIECKUQ';
    ...
    const completed = localStorage.getItem('aegisfund_wizard_completed');
    if (!completed) {
      setShowWizard(true);
    }
  }, [refreshTrigger]);
  ...
  const handleCloseWizard = () => {
    localStorage.setItem('aegisfund_wizard_completed', 'true');
    setShowWizard(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-gray-100 flex flex-col relative bg-gradient-radial">
          ...
          <div className="glass-card max-w-md w-full p-6 border border-white/10 relative overflow-hidden space-y-6 shadow-2xl animate-in zoom-in-95 duration-200 rounded-2xl">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold font-outfit text-white">AegisFund Onboarding Wizard</h3>
              </div>
            ...
            {wizardStep === 1 && (
              <div className="space-y-4">
                <div className="p-3 bg-blue-500/10 rounded-lg text-blue-400 text-xs font-semibold leading-relaxed">
                  Welcome to AegisFund! We protect medical and emergency donations using milestone-based escrow smart contracts on Stellar.
                </div>
                <p className="text-xs text-gray-300 leading-relaxed">
                  Unlike traditional platforms where campaigns receive a lump sum upfront, AegisFund locks contributions in a secure escrow. 
      ...
      <footer className="border-t border-white/5 py-8 text-center text-xs text-gray-500 mt-12 pb-16 sm:pb-8">
        <p>© 2026 AegisFund platform. Powered by Soroban & Stellar.</p>
