...
export const CampaignDetail: React.FC<CampaignDetailProps> = ({ campaignId, address, onBack, onRefresh }) => {
  ...
  const handleContribute = async (e: React.FormEvent) => {
    ...
    if (useSim) {
      const savedBal = localStorage.getItem(`aegisfund_balance_${address}`);
    ...
    try {
      ...
      if (useSim) {
        localStorage.setItem(`aegisfund_balance_${address}`, String(balance - contribAmount));
      }
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-card p-6 border border-white/10 relative overflow-hidden">
            ...
            <div className="p-3 bg-white/5 border border-white/5 rounded-xl text-xs space-y-2">
              ...
              <p className="text-gray-400"><span className="font-semibold text-gray-300">AegisFund Contract:</span> Deployed on Stellar Testnet</p>
            </div>
          </div>
