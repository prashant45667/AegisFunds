...
const seedSimulationDB = () => {
  if (localStorage.getItem('aegisfund_seeded')) return;
  ...
  localStorage.setItem('aegisfund_campaigns', JSON.stringify(sampleCampaigns));
  localStorage.setItem('aegisfund_contributions', JSON.stringify(sampleContributions));
  localStorage.setItem('aegisfund_seeded', 'true');
};
...
const getSimCampaigns = (): Campaign[] => {
  return JSON.parse(localStorage.getItem('aegisfund_campaigns') || '[]');
};

const saveSimCampaigns = (campaigns: Campaign[]) => {
  localStorage.setItem('aegisfund_campaigns', JSON.stringify(campaigns));
};

const getSimContributions = (): Contribution[] => {
  return JSON.parse(localStorage.getItem('aegisfund_contributions') || '[]');
};

const saveSimContributions = (contribs: Contribution[]) => {
  localStorage.setItem('aegisfund_contributions', JSON.stringify(contribs));
};
...
export const StellarService = {
  ...
  getWalletAddress: async (): Promise<string> => {
    if (getUseSimulation()) {
      let mockAddr = localStorage.getItem('aegisfund_mock_address');
      if (!mockAddr || mockAddr.length !== 56) {
        mockAddr = 'GCHHHKNWLK6KGAVIQD5UEZ3NDLGF4POQVABLL2M3WUR5GVGKOQIECKUQ';
        localStorage.setItem('aegisfund_mock_address', mockAddr);
      }
      return mockAddr;
    }
    ...
    try {
      ...
      for (let id = 1; id <= campaignCount; id++) {
        try {
          ...
          const localMeta = JSON.parse(localStorage.getItem(`aegisfund_meta_${id}`) || '{}');
  ...
  createCampaign: async (
    ...
    try {
      ...
      localStorage.setItem(`aegisfund_meta_${nextId}`, JSON.stringify({ title, description, category, verified_ngo: verifiedNgo }));
